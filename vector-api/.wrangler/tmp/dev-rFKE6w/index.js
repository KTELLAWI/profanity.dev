...
// .wrangler/tmp/bundle-ugWilX/checked-fetch.js
...
// .wrangler/tmp/bundle-ugWilX/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-ugWilX/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
app.get(
  "/",
  async (c2) => {
    return c2.json({ sucess: "the server is working well." }, { status: 200 });
  }
);
app.post("/", async (c2) => {
  if (c2.req.header("Content-Type") !== "application/json") {
    return c2.json({ error: "JSON body expected." }, { status: 406 });
  }
  try {
    const { VECTOR_TOKEN, VECTOR_URL } = env(c2);
    const index = new x({
      url: VECTOR_URL,
      token: VECTOR_TOKEN,
      cache: false
    ...
    let { message } = body;
    if (!message) {
      return c2.json({ error: "Message argument is required." }, { status: 400 });
    }
    if (message.split(/\s/).length > 35 || message.length > 1e3) {
      return c2.json(
        {
